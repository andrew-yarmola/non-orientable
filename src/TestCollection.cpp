...
SL2CI TestCollection::construct_word(string word, Params<XComplex>& params)
{
	SL2CI w(1,0,0,1,false);
	SL2CI G(constructG(params));
	SL2CI g(inverse(G));
...
SL2ACJI TestCollection::construct_word(string word, Params<ACJ>& params)
{
	ACJ one(1), zero(0);
	SL2ACJI w(one, zero, zero, one, false);
	SL2ACJI G(constructG(params));
	SL2ACJI g(inverse(G));
...
bool TestCollection::box_inside_nbd(NamedBox& box, string& var_word)
{
	...
	for (vector<string>::iterator it = qrs.begin(); it != qrs.end(); ++it) {
        if (g_power(*it) > min_power) {
            ...
            SL2ACJI w = construct_word(*it, params);
            if (inside_var_nbd(w)) {
                var_word = *it;
                return true;
            } 
...
bool TestCollection::valid_identity_cyclic(string word, Params<ACJ>& params)
{
	...
	for (string::size_type pos = 0; pos < word.size(); ++pos) {
		...
		SL2ACJI w = construct_word(pword, params);
		if (not_identity(w)) {
			return false;
//        } else {
...
//            SL2ACJI w0_j(construct_word(w0_p, params));
//            for (string::size_type idx_1 = 0; idx_1 < w1.size(); ++idx_1) {
//                string w1_p = w1.substr(idx_1, w1.size()-idx_1) + w1.substr(0, idx_1);
//                SL2ACJI w1_j(construct_word(w1_p, params));
//                if (not_zero(w1_j - w0_j) && not_zero(w1_j + w0_j)) { return false; }
//            }
//        }
//        return true;
//    } else {
...
box_state TestCollection::evaluate_ACJ(string word, Params<ACJ>& params, string& aux_word, vector<string>& new_qrs)
{
    ...
    double one = 1; // Exact
	SL2ACJI w = construct_word(word, params);
        ...
        if (not_para_fix_inf(w)) {
			...
			if (isImpossible) return killed_parabolics_impossible;
                ...
                for (vector<string>::iterator it = mandatory.begin(); it != mandatory.end(); ++it) {
                    SL2ACJI w_sub = construct_word(*it, params);
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                for (int j = 0; j < 4; ++j) {
                    ...
                    if (i == 0 && j == 0) {
                        ...
                        ACJ L_k = L * N_k + M_k;
                        w_k = SL2ACJI(w.a - L_k * w.c, w.b - L_k * w.d, w.c, w.d, w.inv); // Cheaper than constucting new word
...
//                        SL2ACJI new_w_k = construct_word(word_k, params);
//                        SL2ACJI gah_k = constructT(params, - int(M_k), - int(N_k)) * w;
